...
const MobileMenu = ({ navMenu, setNavMenu, navLinks }: Props) => {
  return (
    ...
    <div
      ...
      } fixed top-0 w-full max-w-xs h-screen z-25 py-10 px-6 flex flex-col bg-[#111111] border-l-2 border-primary text-lg tracking-wider ease-in-out duration-300`}
    >
      <div className="flex flex-col mt-4">
        ...
        {navLinks.map((navLink) => (
          <Link
            key={navLink.name}
            href={navLink.url}
            className="py-2 px-3 flex items-center rounded-lg hover:bg-neutral-800 font-semibold hover:text-primary custom-transition"
